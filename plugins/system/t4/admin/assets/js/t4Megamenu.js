	...
	$.extend(T4AdminMegamenu, {
  	...
  	renderMenuItem: function($list,$parent){
		...
		ChangeItems: function(data){
			...
			var dataName = data.data('name'),ChoiseMenuType = $('.menu_type').val(),
...
/*			$(document).on('change', '.t4-input-check-mega',function(){
		...
		updateMegamenu: function(){
			var $megaBuilder = $('.t4-menu-layout-builder'),$megaVal = $('[name="jform[params][navigation_mega_settings]"]').val();
		...
		renderMegaMenu: function($mega){
			...
			var $menuType = Object.keys($mega),$layoutMega = '',$items = Object.keys($mega[$menuType]);

			if($items.length){
				for (var i = 0; i < $items.length; i++) {
					var $dataItem = $mega[$menuType][$items[i]],$megabuild = $dataItem.megabuild ? $dataItem.megabuild : "0",$style = "style='display:none;'", $checked = '';
