jQuery(document).ready(function($) {
	...
	$('body').on('click', '.custom-color-list .can-edit label', function() {
	  var $parentGroup = $(this).closest('.control-group'), $colorname = $parentGroup.data('name');
		makefocus();
	...
	var doEdit = function(btn) {
		var $btn = $(btn), $parentGroup = $btn.closest('.control-group');
		var allName = allNameColor() || [];
	...
	var updateColors = function(btn) {
		...
		var $btn = $(btn),value = $btn.val(),nameColors = $btn.attr('name');
		$(document).find('.t4-input-color').each(function(){
			var $input = $(this),$val = $(this).data('val');
	...
	var doRemove = function (btn) {
		var $btn = $(btn), url = location.pathname + '?option=com_ajax&plugin=t4&format=json&t4do=Customcolors&id=' + tempId,
					...
					$.post(url, {task: 'remove', name: name, type:type}).then(function(response) {
						if (response.ok && response.status) {
							...
							input_selected_color.each(function(){
								var $input = $(this),$name_color = $(this).data('val');
