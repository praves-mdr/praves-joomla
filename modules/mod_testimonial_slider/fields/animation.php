...
*/
    ...
    class JFormFieldAnimation extends JFormField {
        ...
        public function makeInput($item, $name, $value=array())
        {
            ...
            $html='<div id="jbe-animation-setting-panel">';
                        ...
                        foreach($fieldset->field as $field)
                        {
                            ...
                            switch((string)$field['type'])
                            {
                                case 'text':
                                    ...
                                    name="'.$fieldname.'"
                                    style="'.$field['style'].'"
                                    value="'.$fieldvalue.'">  <div class="jbe-animation-unit">'.$field['unit'].'</div>  </li>';
                                    break;
        ...
        protected function getInput()
        {
            ...
            if( !isset($this->params['jbe_style'] ))
            {
                // JBE_SLIDER_DEFAULT  difined at fields/tmpl.php file :) 
                $this->params['jbe_style'] = JBE_SLIDER_DEFAULT;
            }
            ...
            $defaultStyle = simplexml_load_file( $tmpl.'/'.$this->params['jbe_style'].'/config.xml' );
                    ...
                    foreach( $xml->files->admin->filename as $file)
                    {
                        if( isset($file['type']) and $file['type']=='javascript' )
                        {
                            if( isset($file['source']) and $file['source']=='external')
                            {
                                ...
                                $doc->addScript(    JURI::root(true).'/modules/mod_testimonial_slider/tmpl/'.$folder.'/'.$file );
                            }
                        }  
                        ...
                        if( isset($file['type']) and $file['type']=='stylesheet' ){
                            ...
                            if( isset($file['source']) and $file['source']=='external')
                            {
                                ...
                                $doc->addStyleSheet( JURI::root(true).'/modules/mod_testimonial_slider/tmpl/'.$folder.'/'.$file );
                            }
                        }
                    }
                }
            ...
            return $this->makeInput($defaultStyle->config, $this->params['jbe_style'], $this->params['animation']);
        }
}